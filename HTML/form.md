...
`for`속성에 `<label>`태그의 형제 요소인 `<input>`태그의 `id`값을 넣어야한다. 한편 `<label>`태그의 자식 요소로 `<input>`태그를 넣어준다면 `for`,`id`를 사용하지 않고 동일한 결과를 얻을 수 있다.

`<label>`태그의`for`값과 `<input>`태그의 `id`값만 일치하면 `<label>`태그와 `<input>`태그의 순서는 상관이 없다.

`<label>`태그,`<input>`태그를 블록 요소로 바꾸려면 `<div>`태그로 감싸준다.
...
> **📌`name`과 `id`**
>
> `name`값은 같은 `<form>`태그 내에서 중복되면 안되지만, 다른 `<form>`태그라면 한 문서에서 중복 가능하다.  
> `id`의 값은 한 문서에서 중복되면 안된다.

## 3. `<fieldset>`태그, `<legend>`태그

`<fieldset>`태그으로 감싸서 웹 양식의 하나 이상의 컨트롤을 묶는다.

`<filedset>`태그의 첫 번째 자식 요소인 `<legend>`태그를 통해 묶은 콘텐츠에 설명을 붙일 수 있다.

`<input>`태그의 속성으로 `disabled`를 넣어 입력 컨트롤을 막을 수 있는데, 이 `disabled`를 `<fieldset>`태그의 속성으로 넣어 해당 fieldset의 모든 자손 입력창을 막을 수 있다.

`<fieldset>`태그도 블록 요소이므로 하나의 컨트롤을 `<div>`태그와 중복으로 사용할 필요는 없다.
...
`<input>`태그의 `type` 속성 키워드인 `submit`, `reset`, `button`으로 만든 버튼들과 동일한 특성을 사용할 수 있다. `<button>`태그의 `type`으로도 `submit`, `reset`, `button` 키워드를 사용할 수 있다.

`<input>`태그의 `type="button"`은 `value`속성을 통해 문자열로 버튼 이름을 변경할 수 있지만, `<button>`태그의 `type="button"`은 태그 내부에 자식 요소로서 이름을 입력하면 해당 이름으로 버튼이 나타난다. 자식 요소를 가진다는 점에서 `<button>`태그로 버튼을 생성하는 것이 스타일링 측면에서 유리하다.

`<button>`태그의 `value` 속성은 해당 버튼의 초깃값(initial value)를 명시한다.

## 6. `<select>`태그, `<option>`태그, `<optgroup>`태그

`<select>`태그는 자식 태그로 `<option>`태그를 가져서 옵션 메뉴을 제공하는 컨트롤을 나타낸다.

`<option>`태그에 `selected`속성을 넣으면 선택 시 초기값으로 설정된다. `checked`속성과 같은 기능을 한다.

`<optgroup>`태그를 통해 옵션들을 묶어줄 수 있고, `label`속성을 이용해 옵션 그룹의 이름을 지정할 수 있다.
...
`<select>`태그와 달리 선택지가 고정되어 있지 않고 추천만 해준다.

`<input>`태그의 `list`속성과 같이 쓰인다. `list`값과 `<datalist>`태그의 `id`값이 일치해야 한다.
...
출력되는 텍스트는 태그 내부의 형식을 그대로 유지한다. `<pre>`태그와 유사하다.

`<input>`태그의 `text`타입과 달리 자식요소를 가질 수 있다.
