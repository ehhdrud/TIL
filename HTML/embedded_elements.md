...
`src` 대신 자식 태그로 `<source>`태그를 사용해도 된다. `<source src="">`형태로 사용하고, 닫는 태그는 없다.
...
`constrols`, `autoplay`, `muted`, `loop`등 `<video>`태그의 속성들을 사용할 수 있다. `autoplay`속성은 비디오와 마찬가지로 소리가 없을 경우에만 동작한다.
