...
`repeat()` 함수 표기법을 사용할 수도 있다. 첫 번째 인자에는 몇 개의 행 또는 열을 가질 것인지, 두 번째 인자에는 크기를 명시한다.

### 1.3. `grid-templete-areas` 속성

`grid-area`속성을 사용해 각 요소마다 `grid-templete-areas` 속성에서 사용할 수 있는 이름을 지정하고, `grid-area` 속성의 값을 `spacing`으로 구분하여 문자열로 작성하여 그리드 아이템들을 배치한다. `grid-area` 속성으로 이름을 지정할 때 적고 동일한 글자수를 가지는 것이 가독성에 유리하다.
...
### 2.1. `grid-row`와 `grid-column` 단축 속성
...
*차지하고자 하는 행 또는 열의 개수*를 속성값에 명시하고 싶다면, 앞 속성값에는 *행과 열을 나누는 선의 고유 번호*를 작성하고 뒤 속성값에 `span N`을 통해 N개의 *차지하고자 하는 행 또는 열*을 작성한다.
...
fraction의 약어로, 그리드 컨테이너의 여유 공간을 비율로 나눠 설정한다.

예를 들어 `1fr 2fr 1fr`라고 작성하면 세개의 행 또는 열이 1:2:1의 비율을 갖는다. 다른 단위와 혼합해서 사용할 수도 있다.

### 3.2. `min-content`와 `max-content` 키워드
...
`auto-fill` 키워드는 다음에 채워질 그리드 아이템이 없다면 `minmax()`를 같이 사용하더라도 여유 공간을 채울 수 없지만 `auto-fit` 키워드는 해당 그리드 컨테이너 안의 아이템의 크기를 조절해 여유 공간을 채운다.
