...
주로 `content`에 빈 문자열(`""`)을 넣고 고유영역을 `position: absolute`를 통해 없앤 후 구분선, 구분바, 말풍선 테두리 등을 만들 때 사용한다.
...
`spacing`을 이용해 첫 번째 요소의 하위에 존재하는 모든 요소에 스타일을 적용한다.

### 3.3. 자식 선택자 결합

`>`를 이용해 첫 번째 요소의 바로 아래 자식 요소에 스타일을 적용한다.

### 3.4. 형제 선택자 결합

`~` 혹은 `+`를 이용해 첫번째 요소와 형제 관계에 있는 요소를 지정한다. 첫번째 요소는 두번째 요소보다 위쪽에 위치해야 한다.

#### 3.4.1. 일반 형제 선택자 결합

`~`를 이용해 첫 번째 요소와 형제 관계에 있는, 아래 쪽의 요소에 스타일을 적용한다.

#### 3.4.2. 인접 형제 선택자 결합

`+`를 이용해 첫 번째 요소와 형제 관계에 있는, 바로 아래 쪽에 인접한 요소에 스타일을 적용한다.

## 4. 범용 선택자

`*`를 이용해 모든 요소를 지정한다.

선택자 결합 시 유용하게 사용할 수 있다.
