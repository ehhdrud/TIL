...
```
<yejineee님 벨로그>
실행 컨텍스트는 3가지 프로퍼티를 가지고 있다. 변수 객체(VO), 스코프 체인(SC), thie value다.

1. 변수 객체(VO)

parameter와 argurment의 정보(FEC의 경우), 함수 선언(함수 표현식은 제외), 변수 이렇게 3가지 정보를 담는다.
...
프로퍼티의 인수 정보를 배열로 가지고 있는 arguments object가 추가된다.
