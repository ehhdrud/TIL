...
아래는 클래스 컴포넌트에서 폼 엘리먼트의 값을 관리하는 예제이다.
...
제어 컴포넌트는 리액트 컴포넌트에서 폼 데이터를 다루지만, 비제어 컴포넌트는 DOM 자체에서 폼 데이터가 다루어진다. 예를 들어, `<input>` 요소의 value 속성은 상태로 관리하는 것이 아니라 DOM에서 직접 값을 가져와 처리하도록 한다. DOM 요소에 직접적으로 접근하기 위해서는 `ref`를 사용한다.
