...
여러 비동기 작업을 순서대로 실행하려면 각각의 비동기 호출을 기다리는 방법을 사용해야 한다.

map 함수 내부에서 await 키워드를 사용하더라도 map 함수 자체는 비동기 작업을 기다리지 않고 즉시 모든 요청을 시작하기 때문에 여러 요청이 동시에 이뤄져 순서가 보장되지 않는다.

아래와 같이 map 대신 for 루프를 사용하여 문제를 해결하였다.
